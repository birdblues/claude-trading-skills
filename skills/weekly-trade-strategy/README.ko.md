...
## <a name="english"></a>English (한국어 번역)

### 개요

Claude Agents를 사용해 미국 주식시장용 주간 트레이딩 전략 블로그 글을 자동 생성하는 AI 에이전트 시스템입니다. 시스템은 단계별 차트 분석, 시장 환경 평가, 뉴스 분석을 수행해 겸업 트레이더를 위한 실행 가능한 전략 리포트를 생성합니다.

### 핵심 기능

- **Technical Analysis**: VIX, 금리, breadth 지표, 주요 지수, 원자재의 주간 차트 분석
- **Market Environment Assessment**: 버블 리스크 탐지, 심리 분석, 섹터 로테이션 분석
- **News & Event Analysis**: 최근 10일 뉴스 영향 평가, 향후 7일 경제지표와 실적 이벤트 전망
- **Weekly Strategy Blog Generation**: 3개 분석 리포트를 200-300행 Markdown 전략 글로 통합
- **Medium-Term Strategy Report** (Optional): 18개월 Druckenmiller 스타일 4시나리오(Base/Bull/Bear/Tail Risk) 전략 리포트

### 사전 요건

- **Claude Code CLI** 또는 **Claude Desktop**
- 다음 Claude 스킬을 사용할 수 있어야 합니다:
  - `technical-analyst`
  - `breadth-chart-analyst`
  - `sector-analyst`
  - `market-environment-analysis`
...
1. 저장소를 클론합니다.
2. `FMP_API_KEY`를 포함한 `.env` 파일을 생성합니다.
3. 날짜 폴더를 생성합니다: `mkdir -p charts/2025-11-03 reports/2025-11-03`
4. 차트 이미지를 `charts/2025-11-03/`에 배치합니다.
5. Claude Code/Desktop에서 전체 워크플로우를 실행합니다(상단 한국어 섹션의 상세 프롬프트 참조).

### 프로젝트 구조

상세 구조는 상단 한국어 섹션을 참고하세요.

### 에이전트

- **technical-market-analyst**: 차트 기반 기술적 분석
- **us-market-analyst**: 시장 환경 및 버블 리스크 평가
- **market-news-analyzer**: 뉴스 영향 및 이벤트 전망 분석
- **weekly-trade-blog-writer**: 최종 블로그 글 생성
- **druckenmiller-strategy-planner** (Optional): 4시나리오 기반 중장기(18개월) 전략 플래닝

### 문서

상세 실행 절차는 `CLAUDE.md`를 참고하세요.
...
Pull Request를 환영합니다. 큰 변경의 경우 먼저 Issue를 열어 변경 방향을 논의해 주세요.

---

## 감사의 말

이 프로젝트는 금융 시장 분석을 위해 Claude의 고급 AI 기능을 활용합니다. 생성되는 모든 트레이딩 전략은 교육 목적이며, 금융 자문으로 간주되어서는 안 됩니다.
