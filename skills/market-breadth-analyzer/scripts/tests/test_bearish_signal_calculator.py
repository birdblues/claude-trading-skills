...
class TestBaseScore:
    ...
    def test_signal_off_trend_down(self, make_row):
        rows = [make_row(Bearish_Signal=False, Breadth_200MA_Trend=-1,
                         Breadth_Index_8MA=0.55, Breadth_Index_200MA=0.50)]
        result = calculate_bearish_signal(rows)
        assert result["base_score"] == 50

    def test_signal_on_trend_up(self, make_row):
        rows = [make_row(Bearish_Signal=True, Breadth_200MA_Trend=1,
                         Breadth_Index_8MA=0.40)]
        result = calculate_bearish_signal(rows)
...
class TestPinkZone:
    ...
    def test_pink_zone_detected(self, make_row):
        rows = [make_row(Breadth_200MA_Trend=-1,
                         Breadth_Index_8MA=0.45, Breadth_Index_200MA=0.50,
    ...
    def test_pink_zone_days_counted(self, make_row):
        rows = [
            make_row(Date="2025-01-01", Breadth_200MA_Trend=1,
                     Breadth_Index_8MA=0.55, Breadth_Index_200MA=0.50),
            make_row(Date="2025-01-02", Breadth_200MA_Trend=-1,
                     Breadth_Index_8MA=0.48, Breadth_Index_200MA=0.50,
                     Bearish_Signal=False),
            make_row(Date="2025-01-03", Breadth_200MA_Trend=-1,
                     Breadth_Index_8MA=0.46, Breadth_Index_200MA=0.50,
                     Bearish_Signal=False),
        ]
        result = calculate_bearish_signal(rows)
