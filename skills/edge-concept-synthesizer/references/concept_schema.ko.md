...
`edge_concepts.yaml`은 detection과 strategy design 사이의 추상화를 담습니다.
...
- 추상화에는 `thesis`와 명시적인 `invalidation_signals`가 모두 포함되어야 합니다.
- `export_ready_v1`는 추천 family가 현재 pipeline interface v1에서 지원될 때만 true여야 합니다.
