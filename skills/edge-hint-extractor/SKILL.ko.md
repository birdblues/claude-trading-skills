...
3. 선택적으로 LLM 아이디어로 힌트를 보강합니다(두 가지 방법 중 택일):
   - a. `--llm-ideas-cmd` — 외부 LLM CLI에 데이터를 파이프합니다(subprocess).
   - b. `--llm-ideas-file PATH` — 미리 작성된 YAML 파일에서 힌트를 로드합니다(Claude Code에서 Claude가 직접 힌트를 생성하는 워크플로용).
4. 생성된 `hints.yaml`을 concept synthesis 또는 auto detection에 전달합니다.

참고: `--llm-ideas-cmd`와 `--llm-ideas-file`은 상호 배타적입니다.
...
Rule + LLM 보강 (미리 작성된 파일, Claude Code용):
