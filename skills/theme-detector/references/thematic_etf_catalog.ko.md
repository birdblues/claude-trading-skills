...
이 카탈로그는 시장 테마와 해당 ETF를 매핑합니다. 테마별 ETF 개수는 라이프사이클 성숙도 평가의 `etf_proliferation_score` 계산에 사용됩니다.

**ETF Proliferation 점수 기준:**
| ETF Count | Score |
|-----------|-------|
| 0         | 0     |
| 1         | 20    |
...
ETF 확산도가 높을수록 개인/기관의 관심이 크고, 테마가 더 성숙(과밀)했을 가능성이 높습니다.
...
**노트:**
- 일부 ETF는 여러 테마에 중복 포함됩니다 (예: LIT은 Clean Energy와 Lithium 모두에 포함, IRBO는 AI와 Robotics 모두에 포함)
- AUM Tier: Large (>$1B), Medium ($100M-$1B), Small (<$100M)
- 레버리지 ETF(LABU, CLDL)는 완결성을 위해 포함했지만, 테마 노출 측정용으로는 권장하지 않습니다
- 신규 ETF 상장 및 상장폐지를 반영하려면 카탈로그를 분기별로 업데이트해야 합니다
