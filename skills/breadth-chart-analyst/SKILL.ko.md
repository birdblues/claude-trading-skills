...
다음 상황에서 이 스킬을 사용합니다:
- 사용자가 S&P 500 Breadth Index (200-Day MA based) 차트 이미지를 제공했을 때
- 사용자가 US Stock Market Uptrend Stock Ratio 차트 이미지를 제공했을 때
- 사용자가 시장 breadth 평가 또는 시장 건전성 평가를 요청할 때
- 사용자가 breadth 지표 기반 중기 전략 포지셔닝을 물을 때
- 사용자가 swing trading을 위한 단기 전술 타이밍 신호가 필요할 때
- 사용자가 전략+전술 결합 시장 전망을 원할 때

다음 상황에서는 이 스킬을 사용하지 않습니다:
- 개별 종목 분석 요청 (대신 `us-stock-analysis` 스킬 사용)
- breadth 차트 없이 섹터 로테이션 분석 필요 (대신 `sector-analyst` 스킬 사용)
- 뉴스 기반 시장 분석 요청 (대신 `market-news-analyst` 스킬 사용)

## 사전 요구사항

- **필수 차트 이미지**: 사용자가 아래 breadth 차트 중 하나 또는 둘 다 제공해야 함
  - Chart 1: S&P 500 Breadth Index (200-Day MA based)
  - Chart 2: US Stock Market Uptrend Stock Ratio
- **API Key 불필요**: 이 스킬은 사용자가 제공한 이미지만 분석하며 외부 데이터 소스가 필요 없음
- **언어**: 모든 분석과 출력은 영어로 수행

## 출력

이 스킬은 `reports/` 디렉터리에 markdown 분석 보고서를 생성합니다:
...
보고서에는 executive summary, 현재 수치, 신호 식별, 확률 기반 시나리오 분석, 트레이더 유형별 실행 가능한 포지셔닝 권고가 포함됩니다.
...
2. ✓ **Latest Data Trend Analysis**: Step 4.1.5를 충분히 수행했는지 - 최근 3-5개 데이터 포인트로 CURRENT 추세 방향 판단
3. ✓ **Trend Direction Accuracy**: 명시한 8MA 기울기(Rising/Falling/Flat)가 과거 움직임이 아니라 RIGHTMOST 데이터 포인트를 반영하는지
4. ✓ **Failed Reversal Check**: trough 식별 시, 최신 궤적으로 반전 유지/실패를 명시적으로 검증했는지
5. ✓ **Specific Values**: 모든 수치가 구체적 퍼센트/레벨인지
6. ✓ **Signal Status**: 활성 신호(CONFIRMED BUY / DEVELOPING / FAILED / SELL / WAIT)를 명확히 식별했는지
7. ✓ **Strategy Alignment**: 권고가 백테스트 전략 및 확인 요건과 정합적인지
8. ✓ **Probabilities**: 시나리오 확률 합이 100%인지
9. ✓ **Actionable**: 트레이더 유형별로 실행 가능한 권고인지
10. ✓ **Context**: 과거 유사 상황과의 비교가 포함되었는지
11. ✓ **Risk Management**: 무효화 레벨과 리스크 요인이 명확한지
