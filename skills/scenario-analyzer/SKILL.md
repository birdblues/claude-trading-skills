...
description: |
  ...
  scenario-analyst 에이전트로 주 분석을 실행하고,
...
이 스킬은 뉴스 헤드라인을 기점으로 중장기(18개월) 투자 시나리오를 분석합니다.
2개의 전문 에이전트(`scenario-analyst`와 `strategy-reviewer`)를 순차 호출하여,
다각적 분석과 비판적 리뷰를 통합한 포괄적 리포트를 생성합니다.

## When to Use This Skill

다음의 경우에 이 스킬을 사용하세요:

- 뉴스 헤드라인으로부터 중장기 투자 영향을 분석하고 싶을 때
- 18개월 후 시나리오를 복수 구축하고 싶을 때
- 섹터·종목에 대한 영향을 1차/2차/3차로 정리하고 싶을 때
- 세컨드 오피니언을 포함한 포괄적 분석이 필요할 때
- 한국어 리포트 출력이 필요할 때
...
| 카테고리 | 예 |
|---------|-----|
| 금융 정책 | FOMC, ECB, BOJ, 금리 인상, 금리 인하, QE/QT |
| 지정학 | 전쟁, 제재, 관세, 무역 마찰 |
| 규제·정책 | 환경 규제, 금융 규제, 독점 금지법 |
| 테크놀로지 | AI, EV, 재생 에너지, 반도체 |
| 원자재 | 원유, 금, 구리, 농산물 |
| 기업·M&A | 인수, 파산, 실적, 업계 재편 |

#### Step 1.3: 레퍼런스 읽기

이벤트 타입에 따라 관련 레퍼런스를 읽어 들입니다:
...
Agent tool을 사용하여 메인 분석 에이전트를 호출합니다.

```
Agent tool:
- subagent_type: "scenario-analyst"
- prompt: |
    다음 헤드라인에 대해 18개월 시나리오 분석을 실행해주세요.
...
**저장처:** `reports/scenario_analysis_<topic>_YYYYMMDD.md`
...
## 4. 포지티브 영향이 예상되는 종목 (3-5종목)
[종목 테이블]

## 5. 네거티브 영향이 예상되는 종목 (3-5종목)
[종목 테이블]
...
- `references/sector_sensitivity_matrix.md` - 섹터 감응도 매트릭스
...
### 시간축
- 시나리오는 **18개월**을 대상
- 0-6개월/6-12개월/12-18개월의 3개 페이즈로 기술
