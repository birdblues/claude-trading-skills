...
분석을 진행하기 전에, 수집 데이터에 대해 아래 필수 검증을 수행합니다:

#### Source Verification
- 핵심 지표(VIX, Treasury yields, major indices)는
  원본 소스 페이지(FRED, Yahoo Finance 등)에서 WebFetch로 정확한 값을 확인합니다.
  WebSearch 요약 텍스트만 신뢰하지 마세요. 요약값은 오래됐거나 부정확할 수 있습니다.

#### Cross-Consistency Check
- 개별 데이터 포인트가 서로, 그리고 전체 시장 내러티브와 일관적인지 확인합니다:
  - 시장이 급락 중이면 → VIX는 낮지 않고 높아야 함(보통 20+)
  - 인플레이션 지표가 뜨거우면 → Treasury yields는 하락보다 상승하는 경향
  - USD가 강세면 → Gold는 보통 역풍(단, 공포 주도 예외)
  - risk-off 환경이면 → 안전자산(Gold, JPY, Treasuries)에 매수 유입
- 어떤 데이터 포인트가 넓은 맥락과 충돌하면, 사용 전에 원소스에서 재검증합니다.
  모순 데이터를 내러티브로 합리화하지 말고, 먼저 데이터를 바로잡으세요.

#### Date Freshness
- 각 데이터의 날짜가 분석 대상 날짜와 일치하는지 확인합니다.
  WebSearch는 출처 날짜를 명확히 표시하지 않고 서로 다른 날짜의 값을 혼합할 수 있습니다.
- 주말/휴일 분석에서는 반드시 "as of [마지막 거래일]"을 명시합니다.
...
**검증 규칙:** VIX, Treasury yields, index closes는 항상
최소 1개 이상의 1차 소스(FRED, Yahoo Finance,
또는 거래소 공식 페이지)에서 WebFetch로 확인합니다.
정확한 수치 데이터에 WebSearch 요약만 쓰는 것은 불충분합니다.
...
- 시장 맥락과 충돌하는 데이터를 합리화하지 마세요. VIX가 급락장 대비 너무 낮거나,
  인플레이션 쇼크 대비 yields가 이상하면,
  데이터가 오래됐거나 부정확할 가능성이 높으므로 작성 전에 재검증하세요.
