...
description: 발행 전 시장 분석 문서와 블로그 글의 데이터 품질을 검증합니다. 가격 스케일 불일치(ETF vs futures), 종목 표기 오류, 날짜/요일 불일치, 배분 합계 오류, 단위 불일치를 점검할 때 사용하세요. 영어와 한국어 콘텐츠를 지원합니다. Advisory mode로 동작하며, 차단이 아닌 사람 검토용 경고를 표시합니다.
---
...
2. **섹션 인식 배분 점검**: 배분 섹션(예: "배분", "Allocation" 헤더,
   "비중", "목표비율" 같은 테이블 컬럼) 내부의 퍼센트만 검사합니다.
   본문의 일반 퍼센트(확률, RSI, YoY 성장률)는 무시합니다.

3. **이중 언어 지원**: 영어/한국어 날짜 형식, 요일 이름, 섹션 헤더를 처리합니다.
