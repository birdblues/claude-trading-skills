...
- **마켓 뉴스 애널리스트** (`market-news-analyst`)
  - WebSearch/WebFetch를 사용한 자동 수집을 통해 지난 10일간의 시장 동향 뉴스 이벤트를 분석. Supabase MCP 연결 시 실시간 속보 DB도 활용.
...
- **마켓 브레드 애널라이저** (`market-breadth-analyzer`)
  - 6컴포넌트 스코어링 시스템(0-100, 100 = 최대 건전성)으로 시장 폭의 건전성을 정량화.
  - 두 가지 데이터 소스: CSV 모드(기본, TraderMonty, API 키 불필요) / FMP 모드(`--source fmp`, S&P 500 종가 기반 자체 계산).
  - 시장이 상승 또는 하락에 얼마나 광범위하게 참여하는지를 측정.
  - FMP 모드는 디스크 캐시로 예산 기반 멀티 런 빌드 지원(무료 티어 호환).

- **업트렌드 애널라이저** (`uptrend-analyzer`)
  - Monty's Uptrend Ratio Dashboard를 사용하여 약 2,800개의 미국 주식을 11개 섹터에 걸쳐 추적하고 시장 폭의 건전성을 진단.
  - 두 가지 데이터 소스: CSV 모드(기본, API 키 불필요) / FMP 모드(`--source fmp`, S&P 500 ~503종목 기반 자체 계산).
...
- **트럼프 평균회귀** (`trump-mean-reversion`)
  - TACO(Trump Always Chickens Out) 프레임워크를 사용하여 Base/Bull/Bear 시나리오 확률을 보정하는 후처리 스킬.
  - 5개 도메인(에너지/유가, 주식시장, 지정학, 무역/관세, 금리)에 걸쳐 Trump Pain Index(0-100)를 산출.
  - 양방향 평균회귀 모델링: 고통 높으면 Bear 축소(TACO 반전 예상), 고통 낮으면 Bull 축소(공격적 정책 확대).
  - 반복 개입의 체감효과와 말 vs 행동 할인율(구두 30%, 행정명령 70%, 정책행동 100%) 내장.
  - 제약: 최대 ±20pp 이동, 5% 하한, 75% 상한. scenario-analyzer 또는 stanley-druckenmiller-investment 뒤에 파이프라인 단계로 통합.
  - FMP API 선택적(`--use-fmp`)으로 실시간 에너지/주식/금리 데이터 수집; 수동 입력만으로도 완전 오프라인 동작.
...
- **리포트 번역기** (`report-translator`)
  - 영어 분석 리포트를 한국어로 번역. 티커, 숫자, 기술 지표명 등 데이터 무결성 보존.
  - 번역 후 data-quality-checker로 데이터 정확성 자동 검증.
  - API 키 불필요 — 금융 용어 가이드라인에 따라 Claude가 번역 수행.
...
| **마켓 브레드 애널라이저** | 🟡 선택 | ❌ 미사용 | ❌ 미사용 | CSV 모드(기본): 무료; FMP 모드(`--source fmp`): FMP 키 필요 |
| **업트렌드 애널라이저** | 🟡 선택 | ❌ 미사용 | ❌ 미사용 | CSV 모드(기본): 무료; FMP 모드(`--source fmp`): FMP 키 필요 |
...
| **트럼프 평균회귀** | 🟡 선택적 | ❌ 미사용 | ❌ 미사용 | FMP으로 실시간 시장 데이터 수집(`--use-fmp`); 수동 입력만으로 오프라인 동작 |
